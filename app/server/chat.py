...
async def _fetch_generated_images_workaround(session: ChatSession, rcid: str) -> list[GeneratedImage]:
    ...
    try:
        ...
        # Create GeneratedImage objects without downloading (optimization)
        # The actual download will happen later in _image_to_base64() when needed
        logger.info(f"🔄 Creating {len(image_urls)} GeneratedImage object(s)...")

        generated_images = []
        for idx, url in enumerate(image_urls):
            try:
                # Create GeneratedImage object with required cookies field
                # No HTTP request here - just object creation
                img = GeneratedImage(
                    url=url,
                ...
                logger.debug(f"✅ Created GeneratedImage object {idx+1}/{len(image_urls)}")

            except Exception as e:
                logger.warning(f"Failed to create GeneratedImage for URL {url}: {e}")
                continue

        logger.info(f"✅ Successfully created {len(generated_images)}/{len(image_urls)} GeneratedImage object(s)")
