...
async def _fetch_generated_images_workaround(session: ChatSession, rcid: str) -> list[GeneratedImage]:
    ...
    try:
        ...
        # Parallel download images and convert to GeneratedImage objects
        logger.info(f"🔄 Downloading {len(image_urls)} image(s) in parallel...")

        async def download_single_image(idx: int, url: str):
            """Download a single image with its own HTTP client"""
            try:
                ...
                logger.warning(f"Failed to download image {idx+1} from {url}: {e}")
                return None

        # Download all images in parallel using asyncio.gather
        download_tasks = [download_single_image(idx, url) for idx, url in enumerate(image_urls)]
        ...
        logger.info(f"✅ Successfully downloaded {len(generated_images)}/{len(image_urls)} image(s)")
